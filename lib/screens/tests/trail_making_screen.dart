...
class _TrailMakingScreenState extends State<TrailMakingScreen> {
  ...
  List<int> nodes = List.generate(totalNodes, (i) => i + 1);
  int currentTarget = 1;
  int mistakes = 0;
  ...
  void onNodeTap(int value) async {
    if (completed) return;
    ...
    if (value == currentTarget) {
      if (currentTarget == totalNodes) {
        ...
        await LocalStore.saveTrailMakingResult(
          elapsedSeconds,
          mistakes,
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Sequencing Activity")),
      body: Padding(
        padding: const EdgeInsets.all(24),
        child: completed ? _resultView() : _testView(),
      ),
    );
  }
      ...
      children: [
        ...
        Expanded(
          child: GridView.count(
            ...
            crossAxisSpacing: 12,
            mainAxisSpacing: 12,
            children: nodes.map((n) => _node(n)).toList(),
          ),
        ),
      ],
    );
  ...
  Widget _node(int number) {
    final bool completedNode = number < currentTarget;

    return GestureDetector(
      onTap: () => onNodeTap(number),
      child: Container(
